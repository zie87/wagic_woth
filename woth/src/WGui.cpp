...
void WGuiItem::Render() {
    WFont* mFont         = WResourceManager::Instance()->GetWFont(Fonts::OPTION_FONT);
    DWORD const oldcolor = mFont->GetColor();
    mFont->SetColor(getColor(WGuiColor::TEXT));
    const float fH     = (height - mFont->GetHeight()) / 2;
    const string trans = _(displayValue);
    const float fW     = mFont->GetStringWidth(trans.c_str());
    const float boxW   = getWidth();
...
WGuiItem::WGuiItem(string _display, u8 _mF)
    : mFlags(_mF)
    , displayValue(std::move(_display))
    , height(20)
    , mFocus(false)
    , width(SCREEN_WIDTH)
    , x(0)
...
void WGuiHeader::Render() {
    WFont* mFont = WResourceManager::Instance()->GetWFont(Fonts::OPTION_FONT);
    // save the current scaling factor.  We don't want the lists to change font size these lists should stay the same no
    // matter what
    const float currentScale = mFont->GetScale();
...
bool WGuiMenu::Leaving(JButton key) {
    ...
    if (key == buttonPrev && currentItem > 0) {
        return false;
    }
...
bool WGuiList::CheckUserInput(JButton key) {
    ...
    if ((key == JGE_BTN_OK) &&
        ...
        const int begin           = (startWindow == -1) ? 0 : startWindow;
        const int end             = (endWindow == -1) ? items.size() : endWindow;
...
string WDecoEnum::lookupVal(int value) {
    ...
    if (edef) {
        const int idx = edef->findIndex(value);
        if (idx != INVALID_ID) {
...
// WGuiMenu
WGuiMenu::WGuiMenu(JButton next, JButton prev, bool dPad, WSyncable* syncme)
    : WGuiItem("")
    , buttonNext(next)
    , buttonPrev(prev)
    , currentItem(-1)
...
bool WGuiMenu::CheckUserInput(JButton key) {
    ...
    if (currentItem >= 0 && currentItem < nbitems) {
        result = items[currentItem]->CheckUserInput(key);
    }
...
void WGuiTabMenu::Render() {
    ...
    mFont->SetScale(0.8f);
    for (auto it = items.begin(); it != items.end(); it++) {
        const float w = mFont->GetStringWidth(_((*it)->getDisplay()).c_str());
...
void WGuiAward::Overlay() {
    ...
    const string s = details;
    if (!s.empty()) {
        const float fW = mFont->GetStringWidth(s.c_str());
        float fH       = mFont->GetHeight();

        if (fH < 16) {
            fH = 18;
        }
        const JQuadPtr button =
        ...
        if (button) {
            r->RenderQuad(button.get(), 10, 12, 0, .5, .5);
        }
...
void WGuiAward::Underlay() {
    ...
    const string n = id ? Options::getName(id) : textId;
    if (!n.empty()) {
...
void WGuiAward::Render() {
    ...
    float myY      = y;
    float fH       = mFont->GetHeight();
    const float fM = fH / 5;  // Font Margin is 20% font height
...
float WGuiImage::getHeight() {
    const JQuadPtr q = source->getImage();
...
void WGuiCardImage::Render() {
    JRenderer* renderer = JRenderer::GetInstance();
    MTGCard* c          = nullptr;
    const Pos p(x + margin, y + margin, 1, 0, 255);
    ...
    if (!source || (c = source->getCard(mOffset.getPos())) == nullptr) {  // No card, use card back.
        ...
        const float scale = p.actZ * 257.f / q->mHeight;
        ...
        if (bThumb) {  // Thumbnail.
            ...
            const JQuadPtr q = source->getImage(mOffset.getPos());

            const int mode = (!q.get() || options[Options::DISABLECARDS].number) ? DrawMode::kText : DrawMode::kNormal;
...
void WGuiListRow::Render() {
    ...
    if (start >= 0) {
        ...
        for (int pos = 0; pos < nbitems; pos++) {
            ...
            const float temp = items[pos]->getHeight() + 3;
            if (temp > tallestRow) {
                tallestRow = temp;
            }
            if (temp > cTallest) {
...
WGuiListRow::WGuiListRow(string n, WSyncable* s) : WGuiList(std::move(n), s) {
...
string WGuiFilters::getCode() {
    ...
    string res;
    vector<WGuiBase*>::iterator row;
    vector<WGuiBase*>::iterator col;
    for (row = list->items.begin(); row != list->items.end(); row++) {
        ...
        if (wgl) {
            ...
            for (col = wgl->items.begin(); col != wgl->items.end(); col++) {
                ...
                if (fi) {
                    const string gc = fi->getCode();
                    if (!res.empty() && !gc.empty() && res[res.size() - 1] != '(') {
                        res += "&";
                    }
                    res += gc;
                }
...
void WGuiFilters::Update(float dt) {
    ...
    if (list) {
        ...
        for (it = wgl->items.begin(); it != wgl->items.end(); it++) {
            ...
            if (!wgfi || wgfi->mState != WGuiFilterItem::STATE_REMOVE) {
                continue;
            }
            SAFE_DELETE(*it);
...
bool WGuiFilters::CheckUserInput(JButton key) {
    if (subMenu && !subMenu->isClosed() && subMenu->CheckUserInput(key)) {
        return true;
    }
...
bool WGuiFilters::isAvailableCode(const string& code) {
    if (!list) {
        return false;
    }
    auto* wgl = dynamic_cast<WGuiList*>(list->Current());
    if (wgl) {
        ...
        for (it = wgl->items.begin(); it != wgl->items.end(); it++) {
            auto* wgfi = dynamic_cast<WGuiFilterItem*>(*it);
            if (!wgfi || wgfi->mState != WGuiFilterItem::STATE_FINISHED) {
                continue;
            }
            if (wgfi->mCode.empty()) {
                continue;
...
void WGuiFilters::addArg(const std::string& display, const std::string& code) {
    if (!subMenu || !isAvailableCode(code)) {
        return;
    }
    subMenu->Add((int)tempArgs.size(), display.c_str());
    tempArgs.emplace_back(display, code);
}
    ...
    case STATE_CHOOSE_TYPE:
        ...
        if (filterType == FILTER_TYPE) {
            ...
            for (int i = Subtypes::LAST_TYPE + 1;; i++) {
                const string s = MTGAllCards::findType(i);
                if (s.empty()) {
                    break;
                }
...
string WGuiFilterItem::getCode() {
    if (mState != STATE_FINISHED || mCode.empty()) {
        return "";
    }
    return mCode;
}
...
void WGuiKeyBinder::setData() {
    ...
    j->ClearBindings();
    for (auto it = items.begin(); it != items.end(); ++it) {
        auto* o = dynamic_cast<OptionKey*>(*it);
        ...
        for (auto jt = it + 1; jt != items.end(); ++jt) {
            ...
            if (C(*it)->from == C(*jt)->from) {
                if (confirmedKeys.end() == std::find(confirmedKeys.begin(), confirmedKeys.end(), C(*it)->from)) {
                    ...
                    if (boundFunctionsList.empty()) {
                        boundFunctionsList.push_back(C(*it)->to);
                    }
...
void WGuiKeyBinder::Render() {
    ...
    if (confirmMenu) {
        ...
        float y    = 20;
        do {
            const size_t t = confirmationString.find_first_of('\n', pos);
            const string s = confirmationString.substr(pos, t - pos);
