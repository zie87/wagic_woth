...
GuiHandSelf::GuiHandSelf(GameObserver* observer, MTGHand* hand)
    ...
    , state(Closed)
    , backpos(ClosedX, SCREEN_HEIGHT - 250, 1.0, 0, 255)
...
bool GuiHandSelf::CheckUserInput(JButton key) {
    const JButton trigger = (options[Options::REVERSETRIGGERS].number ? JGE_BTN_PREV : JGE_BTN_NEXT);
    if (trigger == key) {
