...
int AIMomirPlayer::getEfficiency(OrderedAIAction* action) {
    ...
    const int efficiency = AIPlayerBaka::getEfficiency(action);

    if (observer->getCurrentGamePhase() < MTG_PHASE_FIRSTMAIN) {
        return 0;
    }
    return efficiency;
}
...
int AIMomirPlayer::momir() {
    ...
    int result                  = 0;
    const int opponentCreatures = getCreaturesInfo(opponent(), INFO_NBCREATURES);
    const int myCreatures       = getCreaturesInfo(this, INFO_NBCREATURES);
    ManaCost* potentialMana     = getPotentialMana();
    int converted               = potentialMana->getConvertedCost();
    SAFE_DELETE(potentialMana);
    int efficiency   = 100;
    const int chance = 1 + (randomGenerator.random() % 100);
...
int AIMomirPlayer::computeActions() {
    ...
    /*
     ...
     little something like this: (a) if you are on the play, hit your Two through Four, skip your Five, and then hit all
     the way to Eight; (b) if you are on the draw and your opponent skips his One, you make Two through Eight; (c) if
     you are on the draw and your opponent hits a One, you match him drop-for-drop for the rest of the game.

     You skip your Five on the play because it is the weakest drop. There are plenty of serviceable guys there, but very
     few bombs compared to other drops the general rule is this: if you want to get to Eight, you have to skip two drops
     on the play and one drop on the draw.
     */
    ...
    if (p == this && observer->mLayers->stackLayer()->count(0, NOT_RESOLVED) == 0) {  // standard actions
        ...
        case MTG_PHASE_FIRSTMAIN: {
            ManaCost* potentialMana = getPotentialMana();
            const int converted     = potentialMana->getConvertedCost();
            SAFE_DELETE(potentialMana);
            ...
            if (converted < 8 || game->hand->nb_cards > 1) {
                ...
                const int canPutLandsIntoPlay = game->playRestrictions->canPutIntoZone(card, game->inPlay);
