...
CardSelector::SelectorMemory::SelectorMemory() : object(nullptr) { x = y = 0; }
...
CardSelector::CardSelector(GameObserver* observer, DuelLayers* duel)
    ...
    , duel(duel)
    , limitor(nullptr)
    , bigpos(300, 145, 1.0, 0.0, 220) {}

void CardSelector::Add(CardSelector::Target* target) {
    if (nullptr == active) {
        if (nullptr == limitor || limitor->select(active)) {
            active = target;
        }
    }
...
void CardSelector::Update(float dt) {
    const float boundary = duel->RightBoundary();
