...
void izstream::open(const char* Filename, streamoff Offset, streamoff Size, int CompMethod) {
    ...
    if (m_CompMethod == CompMethod) {
        if (rdbuf() != nullptr) {
            dynamic_cast<zbuffer*>(rdbuf())->close();
        }
    } else {
    ...
    case DEFLATED:
        ...
        if (!(dynamic_cast<zbuffer*>(rdbuf())->open(Filename, Offset, Size))) {
            setstate(ios::badbit);
            SetCompMethod(-1);
        }
...
int zbuffer_stored::underflow() {
    ...
    const streamoff ToRead = ((m_Size - m_Pos) < BUFFERSIZE) ? (m_Size - m_Pos) : BUFFERSIZE;
