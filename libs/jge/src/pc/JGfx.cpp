...
JTexture::JTexture() : mTexId(-1), mBuffer(nullptr) {}
...
void JRenderer::RenderQuad(JQuad* quad, float xo, float yo, float angle, float xScale, float yScale) {
    ...
    const float width  = quad->mWidth;
    const float height = quad->mHeight;
...
void JRenderer::LoadJPG(TextureInfo& textureInfo,
    ...
    const int tw = getNextPower2(cinfo.output_width);
    const int th = getNextPower2(cinfo.output_height);
...
static void PNGCustomReadDataFn(png_structp png_ptr, png_bytep data, png_size_t length) {
    ...
    auto* fileSystem = static_cast<JFileSystem*>(png_get_io_ptr(png_ptr));
...
JTexture* JRenderer::LoadTexture(const char* filename, int mode, int TextureFormat __attribute__((unused))) {
    ...
    } else if (strstr(filename, ".png") != nullptr || strstr(filename, ".PNG") != nullptr) {
        LoadPNG(textureInfo, filename);
    }

    if (textureInfo.mBits == nullptr) {
...
int JRenderer::LoadPNG(TextureInfo& textureInfo,
    ...
    png_set_error_fn(png_ptr, (png_voidp) nullptr, (png_error_ptr) nullptr, PNGCustomWarningFn);
    ...
    if (buffer) {
        ...
        for (y = 0; y < (int)height; y++) {
            ...
            for (x = 0; x < (int)width; x++) {
                ...
                const int a   = (color32 >> 24) & 0xff;
                const int r   = color32 & 0xff;
                const int g   = (color32 >> 8) & 0xff;
                const int b   = (color32 >> 16) & 0xff;
...
JTexture* JRenderer::CreateTexture(int width, int height, int mode __attribute__((unused))) {
    ...
    if (tex) {
        const int size = width * height * sizeof(PIXEL_TYPE);  // RGBA
        BYTE* buffer   = new BYTE[size];
        if (buffer) {
            tex->mFilter    = TEX_FILTER_LINEAR;
            tex->mWidth     = width;
...
void JRenderer::ClearScreen(PIXEL_TYPE color) {
    checkGlError();
    static auto previousColor = RGBA(0xFF, 0xFF, 0xFF, 0xFF);
...
void JRenderer::DrawPolygon(float x, float y, float size, int count, float startingAngle, PIXEL_TYPE color) {
    ...
    const float steps = 360.0f / count;
    size /= 2;
    int i;
