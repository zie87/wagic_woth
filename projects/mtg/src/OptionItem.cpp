...
// OptionItem
OptionItem::OptionItem(int _id, string _displayValue) : WGuiItem(std::move(_displayValue)), id(_id) { mFocus = false; }
...
void OptionInteger::Render() {
    ...
    if (maxValue == 1) {
        ...
        if (value == defValue && !strDefault.empty()) {
...
void OptionProfile::populate() {
    const string temp = options[Options::ACTIVE_PROFILE].str;
...
void OptionProfile::Render() {
    ...
    WFont* mFont        = WResourceManager::Instance()->GetWFont(Fonts::OPTION_FONT);
    mFont->SetScale(1);
    const int spacing = 2 + (int)mFont->GetHeight();
    ...
    const string filename = buf;
    const JQuadPtr avatar = WResourceManager::Instance()->RetrieveTempQuad(filename, TEXTURE_SUB_EXACT);
...
// OptionThemeStyle
OptionThemeStyle::OptionThemeStyle(string _displayValue) : OptionSelect(Options::GUI_STYLE, std::move(_displayValue)) {
    Reload();
    initSelections();
}
...
// OptionLanguage
OptionLanguage::OptionLanguage(string _displayValue) : OptionSelect(Options::LANG, std::move(_displayValue)) {
    Reload();
    initSelections();
};
...
void OptionDirectory::Reload() {
    ...
    for (size_t i = 0; i < subfolders.size(); ++i) {
        const string filename = root + "/" + subfolders[i] + "/" + type;
...
OptionDirectory::OptionDirectory(const string& root, int id, string displayValue, const string& type)
    : OptionSelect(id, std::move(displayValue))
    , root(root)
    , type(type) {
    vector<string> subfolders = JFileSystem::GetInstance()->scanfolder(root);
    ...
    for (size_t i = 0; i < subfolders.size(); ++i) {
        ...
        const string filePath = buildFilePath(path, type);
...
JQuadPtr OptionTheme::getImage() {
    char buf[512];
    const string val = selections[value];
    if (val == "Default") {
...
void OptionTheme::Render() {
    ...
    const JQuadPtr q = getImage();
    if (q) {
        const float scale = 128 / q->mHeight;
    ...
    if (bChecked && !author.empty()) {
        mFont->SetColor(getColor(WGuiColor::TEXT_BODY));
        mFont->SetScale(0.8f);
        const float hi = mFont->GetHeight();
...
void OptionKey::Render() {
    ...
    if (LOCAL_KEY_NONE == from) {
        const string msg = _("New binding...");
        ...
        if (rep2.second) {
            const float ratio = 16.0f / rep2.second->mHeight;
