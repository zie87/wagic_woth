...
TargetChooser* TargetChooserFactory::createTargetChooser(string s, MTGCardInstance* card, MTGAbility* ability) {
    ...
    if (found == 0) {
        ...
        if (ability) {
            target = dynamic_cast<MTGCardInstance*>(ability->target);
        }
    ...
    if (found == 0) {
        if (s.length() > 7 && s.find('[') == 7) {
            const string s1 = s.substr(7, s.find(']'));
            if (s1.find("to") != string::npos) {
                return NEW TriggerTargetChooser(observer, WEvent::TARGET_TO);
            }
            if (s1.find("from") != string::npos) {
                return NEW TriggerTargetChooser(observer, WEvent::TARGET_FROM);
            }
        }
    ...
    if (found != string::npos) {
        int maxtargets       = 1;
        const size_t several = s.find("<anyamount>");
    ...
    if (found != string::npos) {
        const int maxtargets = TargetChooser::UNLITMITED_TARGETS;
    ...
    if (found != string::npos) {
        ...
        while (!s2.empty()) {
            ...
            if (zoneName == "*") {
                ...
                const int zone = MTGGameZone::zoneStringToId(zoneName);
                if (zone) {
                    zones[nbzones++] = zone;
                }
            }
    ...
    // max targets allowed
    const size_t limit = s1.find('<');
    if (limit != string::npos) {
        const size_t end = s1.find('>', limit);
        string howmany;
        if (end != string::npos) {
            howmany                 = s1.substr(limit + 1, end - limit - 1);
            const size_t uptoamount = howmany.find("upto:");
    ...
    while (!s1.empty()) {
        ...
        if (found != string::npos) {
            ...
            while (!attributes.empty()) {
                ...
                // creature dealt damage to controller
                    ...
                    const size_t start  = attribute.find("share!");
                    const size_t end    = attribute.rfind('!');
                    const string CDtype = attribute.substr(start + 6, end - start);

                    if (card && card->isSpell() && !card->backupTargets.empty()) {
                    ...
                    if (attribute.find("{any}") != string::npos) {
                        ...
                        const size_t start         = attribute.find('{');
                        const size_t end           = attribute.find('}');
                        const string counterString = attribute.substr(start + 1, end - start - 1);
        ...
        if (cd) {
            ...
            if (!tc) {
                ...
                (dynamic_cast<TypeTargetChooser*>(tc))->addType(typeName.c_str());
...
TargetChooser* TargetChooserFactory::createTargetChooser(MTGCardInstance* card) {
    ...
    int id         = card->getId();
    const string s = card->spellTargetType;
    if (card->alias) {
        id = card->alias;
...
bool TargetChooser::canTarget(Targetable* target, bool withoutProtections) {
    ...
    if (auto* card = dynamic_cast<MTGCardInstance*>(target)) {
        ...
        if (source && targetter && card->isInPlay(observer) && !withoutProtections) {
            if (card->has(Constants::SHROUD)) {
                return false;
            }
            if (card->protectedAgainst(targetter)) {
                return false;
            }
            if (card->CantBeTargetby(targetter)) {
                return false;
            }
            if ((targetter->controller() != card->controller()) && card->has(Constants::OPPONENTSHROUD)) {
                return false;
            }
        }
        return true;
    }
    if (dynamic_cast<Interruptible*>(target)) {
        return true;
    }
...
int TargetChooser::ForceTargetListReady() {
    const int state = targetsReadyCheck();
    if (state == TARGET_OK) {
...
bool CardTargetChooser::canTarget(Targetable* target, bool withoutProtections) {
    ...
    if (!nbzones && !TargetChooser::canTarget(target, withoutProtections)) {
        return false;
    }
    if (nbzones && !TargetZoneChooser::canTarget(target, withoutProtections)) {
        return false;
    }
...
bool TypeTargetChooser::canTarget(Targetable* target, bool withoutProtections) {
    if (!TargetZoneChooser::canTarget(target, withoutProtections)) {
        return false;
    }
    if (auto* card = dynamic_cast<MTGCardInstance*>(target)) {
    ...
    if (auto* action = dynamic_cast<Interruptible*>(target)) {
        if (action->type == ACTION_SPELL && action->state == NOT_RESOLVED) {
            ...
            for (int i = 0; i < nbtypes; i++) {
                ...
                if (MTGAllCards::findType(card->name) == types[i]) {
                    return true;
                }
            }
            return false;
...
bool TargetZoneChooser::canTarget(Targetable* target, bool withoutProtections) {
    if (!TargetChooser::canTarget(target, withoutProtections)) {
        return false;
    }
    if (auto* card = dynamic_cast<MTGCardInstance*>(target)) {
        for (int i = 0; i < nbzones; i++) {
            ...
            if (MTGGameZone::intToZone(observer, zones[i], source, card)->hasCard(card)) {
                return true;
            }
        }
        ...
        if (spell->state == NOT_RESOLVED) {
            MTGCardInstance* card = spell->source;
            for (int i = 0; i < nbzones; i++) {
                if (MTGGameZone::intToZone(Owner->getObserver(), zones[i], source, card)->hasCard(card)) {
                    return true;
                }
            }
        }
...
bool TargetZoneChooser::targetsZone(MTGGameZone* z) {
    for (int i = 0; i < nbzones; i++) {
        if (MTGGameZone::intToZone(Owner->getObserver(), zones[i], source) == z) {
            return true;
        }
    }
    return false;
...
bool TargetZoneChooser::targetsZone(MTGGameZone* z, MTGCardInstance* mSource) {
    ...
    for (int i = 0; i < nbzones; i++) {
        if (MTGGameZone::intToZone(source->owner->getObserver(), zones[i], source) == z) {
            return true;
        }
    }
    return false;
...
bool SpellTargetChooser::canTarget(Targetable* target, bool withoutProtections) {
    ...
    if (spell->state == NOT_RESOLVED) {
        MTGCardInstance* card = spell->source;
        if (card && (color == -1 || card->hasColor(color))) {
            return true;
        }
    }
...
bool SpellOrPermanentTargetChooser::canTarget(Targetable* target, bool withoutProtections) {
    if (auto* card = dynamic_cast<MTGCardInstance*>(target)) {
        if (color == -1 || card->hasColor(color)) {
            return TargetZoneChooser::canTarget(target, withoutProtections);
        }
    } else if (auto* spell = dynamic_cast<Spell*>(target)) {
        if (spell->state == NOT_RESOLVED) {
            MTGCardInstance* card = spell->source;
