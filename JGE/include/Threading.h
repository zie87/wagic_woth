...
struct unique_lock {
    unique_lock(Mutex& inMutex) : mMutex(&inMutex) { mMutex->lock(); }
...
public:
    ...
    mutex() { mID = sceKernelCreateSema("Unnamed", 0, 1, 1, 0); }

    ~mutex() { sceKernelDeleteSema(mID); }
...
public:
    ...
    recursive_mutex() : mRecursionCount(0) { mID = sceKernelCreateSema("Unnamed", 0, 1, 1, 0); }

    ~recursive_mutex() { sceKernelDeleteSema(mID); }
...
class thread {
    ...
    */
    struct CallbackData {
        CallbackData(detail::thread_data_ptr inThreadInfo) : mThreadInfo(inThreadInfo) {}
...
public:
    ...
    void join() { sceKernelTerminateDeleteThread(mThreadProcID); }
...
inline void sleep(boost::posix_time::milliseconds const& time) { sceKernelDelayThread(time * 1000); }
}  // namespace this_thread
}  // namespace boost
